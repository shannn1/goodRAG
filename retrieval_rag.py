...
from transformers.tokenization_utils import PreTrainedTokenizer
from transformers.tokenization_utils_base import BatchEncoding
from transformers.utils import cached_file, logging, requires_backends
from configuration_rag import RagConfig
from tokenization_rag import RagTokenizer
...
class HFIndexBase(Index):
    def __init__(self, vector_size, dataset, index_initialized=False):
        ...
        self._check_dataset_format(index_initialized)
        dataset.set_format("numpy", columns=["embeddings"], dtype="float32")

    def _check_dataset_format(self, with_index):
        if not isinstance(self.dataset, Dataset):
            raise TypeError(f"Expected a datasets.Dataset, got {type(self.dataset)}")

        required_columns = {"title", "text", "embeddings"}
        if not required_columns.issubset(self.dataset.column_names):
            raise ValueError(f"Dataset must contain columns {required_columns}, but got {self.dataset.column_names}")

        if with_index and "embeddings" not in self.dataset.list_indexes():
            raise ValueError("The FAISS index for 'embeddings' is missing.")

    def is_initialized(self):
        return self._index_initialized

    def get_doc_dicts(self, doc_ids: np.ndarray):
        return [self.dataset[doc_id.tolist()] for doc_id in doc_ids]

    def get_top_docs(self, question_hidden_states: np.ndarray, n_docs=5):
        _, ids = self.dataset.search_batch("embeddings", question_hidden_states, n_docs)
        vectors = [
            np.vstack([doc["embeddings"] for doc in self.dataset[indices] if indices >= 0])
            for indices in ids
        ]
        for i in range(len(vectors)):
            if len(vectors[i]) < n_docs:
                padding = np.zeros((n_docs - len(vectors[i]), self.vector_size))
                vectors[i] = np.vstack([vectors[i], padding])
        return np.array(ids), np.array(vectors)
...
class CustomHFIndex(HFIndexBase):
    ...
    @classmethod
    def load_from_disk(cls, vector_size, dataset_path, index_path):
        if not dataset_path or not index_path:
            raise ValueError("Provide valid `dataset_path` and `index_path`.")
        dataset = load_from_disk(dataset_path)
        return cls(vector_size, dataset, index_path=index_path)
...
class RagRetriever:
    ...
    def _build_index(config):
        return CustomHFIndex.load_from_disk(
            config.retrieval_vector_size, config.passages_path, config.index_path
        )
    ...
    def from_pretrained(cls, retriever_name_or_path, indexed_dataset=None, **kwargs):
        ...
        index = indexed_dataset and CustomHFIndex(config.retrieval_vector_size, indexed_dataset) or cls._build_index(config)
        return cls(config, rag_tokenizer.question_encoder, rag_tokenizer.generator, index=index)

    def save_pretrained(self, save_directory):
        if isinstance(self.index, CustomHFIndex):
            if not self.config.index_path:
                self.config.index_path = os.path.join(save_directory, "hf_dataset_index.faiss")
                self.index.dataset.get_index("embeddings").save(self.config.index_path)

            if not self.config.passages_path:
                self.config.passages_path = os.path.join(save_directory, "hf_dataset")
                faiss_index = self.index.dataset._indexes.pop("embeddings")
                self.index.dataset.save_to_disk(self.config.passages_path)
                self.index.dataset._indexes["embeddings"] = faiss_index

        self.config.save_pretrained(save_directory)
        RagTokenizer(self.question_encoder_tokenizer, self.generator_tokenizer).save_pretrained(save_directory)
    ...
    def postprocess_docs(self, docs, input_strings, prefix, n_docs, return_tensors=None):
        def cat_input_and_doc(doc_title, doc_text, input_string, prefix):
            return f"{prefix}{doc_title}{self.config.title_sep}{doc_text}{self.config.doc_sep}{input_string}".replace("  ", " ")
        ...
        encoded = self.generator_tokenizer.batch_encode_plus(
            rag_input_strings,
            max_length=self.config.max_combined_length,
            return_tensors=return_tensors,
            padding="max_length",
    ...
    def retrieve(self, question_hidden_states, n_docs):
        doc_ids, doc_vectors = self.index.get_top_docs(question_hidden_states, n_docs)
        return doc_vectors, doc_ids, self.index.get_doc_dicts(doc_ids)

    def __call__(self, question_input_ids, question_hidden_states, prefix=None, n_docs=None, return_tensors=None):
        n_docs = n_docs or self.n_docs
        prefix = prefix or self.config.generator.prefix
        retrieved_doc_embeds, doc_ids, docs = self.retrieve(question_hidden_states, n_docs)
        input_strings = self.question_encoder_tokenizer.batch_decode(question_input_ids, skip_special_tokens=True)
        context_input_ids, context_attention_mask = self.postprocess_docs(
            docs, input_strings, prefix, n_docs, return_tensors
        )

        if self.return_tokenized_docs:
            tokenized_docs = self.ctx_encoder_tokenizer(
                [d["title"] for doc in docs for d in doc],
                [d["text"] for doc in docs for d in doc],
                truncation=True, padding="longest", return_tensors=return_tensors
            )
            return BatchEncoding({
                "context_input_ids": context_input_ids,
                "context_attention_mask": context_attention_mask,
